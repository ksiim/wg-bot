...
"""
        ...
        subprocess.run(['wg-quick', 'down', os.path.join(self.config_dir, self.server_config.split('.')[0])], check=True)
        subprocess.run(['wg-quick', 'up', os.path.join(self.config_dir, self.server_config.split('.')[0])], check=True)
